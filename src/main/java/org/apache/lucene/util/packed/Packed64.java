 ...
 * the underlying array were always accessed, even for the cases where only 1 or
 * 2 were needed. Even with caching, this had a detrimental effect on
 * performance. Related to this issue, the old implementation used lookup tables
 * for shifts and masks, which also proved to be a bit slower than calculating
 * the shifts and masks on the fly. See
 * https://issues.apache.org/jira/browse/LUCENE-4062 for details.
 * 
 */
class Packed64 extends PackedInts.MutableImpl {
  ...
  public int get(int index, long[] arr, int off, int len) {
    ...
    final PackedInts.Decoder decoder = BulkOperation.of(
        PackedInts.Format.PACKED, bitsPerValue);
  ...
  public int set(int index, long[] arr, int off, int len) {
    ...
    final int originalIndex = index;
    final PackedInts.Encoder encoder = BulkOperation.of(
        PackedInts.Format.PACKED, bitsPerValue);
