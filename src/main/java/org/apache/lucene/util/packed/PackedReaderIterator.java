...
final class PackedReaderIterator extends PackedInts.ReaderIteratorImpl {
  ...
  PackedReaderIterator(PackedInts.Format format, int valueCount,
    ...
    nextValues = new LongsRef(
        new long[iterations * bulkOperation.valueCount()], 0, 0);
  ...
  public LongsRef next(int count) throws IOException {
    ...
    nextValues.length = Math.min(nextValues.longs.length - nextValues.offset,
        count);
