...
/**
 * Simplistic compression for array of unsigned long values. Each value is >= 0
 * and <= a specified maximum value. The values are stored as packed ints, with
 * each value consuming a fixed number of bits.
 * 
 * @lucene.internal
 */
public class PackedInts {
  ...
  private static void checkVersion(int version) {
    if (version < VERSION_START) {
      throw new IllegalArgumentException(
          "Version is too old, should be at least " + VERSION_START + " (got "
              + version + ")");
    } else if (version > VERSION_CURRENT) {
      throw new IllegalArgumentException(
          "Version is too new, should be at most " + VERSION_CURRENT + " (got "
              + version + ")");
    }
  }
  ...
  public enum Format {
     ...
     * <code>overhead per value / bits per value</code>).
     */
   ...
   * <code>acceptableOverheadRatio</code>. </p>
   * <p>
   * The <code>acceptableOverheadRatio</code> parameter makes sense for
   * random-access {@link Reader}s. In case you only plan to perform sequential
   * access on this stream later on, you should probably use
  ...
  public static FormatAndBits fastestFormatAndBits(int valueCount,
    ...
    float acceptableOverheadPerValue = acceptableOverheadRatio * bitsPerValue; // in
                                                                               // bits
    ...
    if (bitsPerValue <= 8 && maxBitsPerValue >= 8) {
      ...
      for (int bpv = bitsPerValue; bpv <= maxBitsPerValue; ++bpv) {
        if (Format.PACKED_SINGLE_BLOCK.isSupported(bpv)) {
          ...
          float acceptableOverhead = acceptableOverheadPerValue + bitsPerValue
              - bpv;
  ...
  public static interface Decoder {
     ...
     * <code>blocks</code>, decode them and write
     * <code>iterations * valueCount()</code> values into <code>values</code>.
     * 
     * @param blocks
     ...
     *          controls how much data to decode
     */
    void decode(byte[] blocks, int blocksOffset, long[] values,
        int valuesOffset, int iterations);
     ...
     *          controls how much data to decode
     */
    void decode(long[] blocks, int blocksOffset, int[] values,
        int valuesOffset, int iterations);
     ...
     *          controls how much data to decode
     */
    void decode(byte[] blocks, int blocksOffset, int[] values,
        int valuesOffset, int iterations);
  ...
  public static interface Encoder {
     ...
     * <code>values</code>, encode them and write
     * <code>iterations * blockCount()</code> blocks into <code>blocks</code>.
     * 
     * @param blocks
     *          the long blocks that hold packed integer values
     * @param blocksOffset
     ...
     * Read <code>iterations * valueCount()</code> values from
     * <code>values</code>, encode them and write
     * <code>8 * iterations * blockCount()</code> blocks into
     * <code>blocks</code>.
     * 
     * @param blocks
     *          the long blocks that hold packed integer values
     ...
     *          controls how much data to encode
     */
    void encode(long[] values, int valuesOffset, byte[] blocks,
        int blocksOffset, int iterations);
     ...
     *          controls how much data to encode
     */
    void encode(int[] values, int valuesOffset, long[] blocks,
        int blocksOffset, int iterations);
     ...
     *          controls how much data to encode
     */
    void encode(int[] values, int valuesOffset, byte[] blocks,
        int blocksOffset, int iterations);
  ...
  public static interface Reader {
     ...
     * from <code>index</code> into <code>arr[off:off+len]</code> and return the
     * actual number of values that have been read.
     */
    int get(int index, long[] arr, int off, int len);

    /**
     * @return the number of bits used to store any given value. Note: This does
     *         not imply that memory usage is {@code bitsPerValue * #values} as
    ...
    /**
     * Expert: if the bit-width of this reader matches one of java's native
     * types, returns the underlying array (ie, byte[], short[], int[], long[]);
     * else, returns null. Note that when accessing the array you must upgrade
     * the type (bitwise AND with all ones), to interpret the full value as
     * unsigned. Ie, bytes[idx]&0xFF, shorts[idx]&0xFFFF, etc.
     */
    Object getArray();
  ...
  public static interface ReaderIterator extends Closeable {
    ...
    /**
     * Returns at least 1 and at most <code>count</code> next values, the
     * returned ref MUST NOT be modified
     */
  ...
  public static interface Mutable extends Reader {
    ...
    /**
     * Save this mutable into <code>out</code>. Instantiating a reader from the
     * generated data will return a reader with the same number of bits per
     * value.
     */
  ...
  static abstract class MutableImpl extends ReaderImpl implements Mutable {
    ...
    public void save(DataOutput out) throws IOException {
      Writer writer = getWriterNoHeader(out, getFormat(), valueCount,
   ...
   * 
   * @param in
   *          the stream to read data from, positioned at the beginning of the
   *          packed values
   * @param format
   ...
   * @lucene.internal
   */
  public static Reader getReaderNoHeader(DataInput in, Format format,
   ...
   *          the number of bits per value
   * @param mem
   *          how much memory the iterator is allowed to use to read-ahead
   *          (likely to speed up iteration)
   * @return a ReaderIterator
   * @see PackedInts#getWriterNoHeader(DataOutput, Format, int, int, int)
   * @lucene.internal
   */
  public static ReaderIterator getReaderIteratorNoHeader(DataInput in,
      Format format, int version, int valueCount, int bitsPerValue, int mem) {
    checkVersion(version);
  ...
  public static ReaderIterator getReaderIterator(DataInput in, int mem)
    ...
    final int valueCount = in.readVInt();
    final Format format = Format.byId(in.readVInt());
    return getReaderIteratorNoHeader(in, format, version, valueCount,
        bitsPerValue, mem);
  }
   ...
   * {@link PackedInts#getWriter(DataOutput, int, int, float)}. </p>
   * <p>
   * The returned reader will have very little memory overhead, but every call
   * to {@link Reader#get(int)} is likely to perform a disk seek.
   * 
  ...
  public static Reader getDirectReader(IndexInput in) throws IOException {
    ...
    final int valueCount = in.readVInt();
    final Format format = Format.byId(in.readVInt());
    return getDirectReaderNoHeader(in, format, version, valueCount,
        bitsPerValue);
  }
   ...
   * All Mutables known by this factory are kept fully in RAM. </p>
   * <p>
   ...
   * implementation is selected whereas {@link PackedInts#FASTEST} will make
  ...
  public static Mutable getMutable(int valueCount, int bitsPerValue,
    ...
    final FormatAndBits formatAndBits = fastestFormatAndBits(valueCount,
   ...
   * value count, and number of bits per value. </p>
   * <p>
   * The resulting stream will be long-aligned. This means that depending on the
   * format which is used, up to 63 bits will be wasted. An easy way to make
   * sure that no space is lost is to always use a <code>valueCount</code> that
   * is a multiple of 64.
   * </p>
   * <p>
   ...
   * @lucene.internal
   */
  public static Writer getWriterNoHeader(DataOutput out, Format format,
   ...
   * count, and number of bits per value. </p>
   * <p>
   * The resulting stream will be long-aligned. This means that depending on the
   * format which is used under the hoods, up to 63 bits will be wasted. An easy
   * way to make sure that no space is lost is to always use a
   ...
   * </p>
   * <p>
   * The <code>acceptableOverheadRatio</code> parameter controls how readers
   * that will be restored from this stream trade space for speed by selecting a
   * faster but potentially less memory-efficient implementation. An
   * <code>acceptableOverheadRatio</code> of {@link PackedInts#COMPACT} will
   * make sure that the most memory-efficient implementation is selected whereas
   * {@link PackedInts#FASTEST} will make sure that the fastest implementation
   * is selected. In case you are only interested in reading this stream
   * sequentially later on, you should probably use {@link PackedInts#COMPACT}.
   * 
   * @param out
   *          the data output
   * @param valueCount
  ...
  public static Writer getWriter(DataOutput out, int valueCount,
      int bitsPerValue, float acceptableOverheadRatio) throws IOException {
    assert valueCount >= 0;

    final FormatAndBits formatAndBits = fastestFormatAndBits(valueCount,
        bitsPerValue, acceptableOverheadRatio);
    final Writer writer = getWriterNoHeader(out, formatAndBits.format,
        valueCount, formatAndBits.bitsPerValue, DEFAULT_BUFFER_SIZE);
    writer.writeHeader();
    return writer;
  }
  ...
  public static int bitsRequired(long maxValue) {
    if (maxValue < 0) {
      throw new IllegalArgumentException("maxValue must be non-negative (got: "
          + maxValue + ")");
    }
   ...
   * bytes.
   */
  public static void copy(Reader src, int srcPos, Mutable dest, int destPos,
      int len, int mem) {
    ...
    if (capacity == 0) {
      ...
      int remaining = 0;
      while (len > 0) {
        final int read = src.get(srcPos, buf, remaining,
            Math.min(len, buf.length - remaining));
        assert read > 0;
        srcPos += read;
        len -= read;
        remaining += read;
