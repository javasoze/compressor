...
public final class RamUsageEstimator {
  ...
  static {
    ...
    // If the scaling is byte-wise and unsafe is available, enable dynamic size
    // measurement for
    // estimateRamUsage().
    ...
    try {
      ...
      if (Math.abs(ofs2 - ofs1) == NUM_BYTES_LONG) {
        ...
        objectHeader = ((Number) objectFieldOffsetM
            .invoke(theUnsafe, baseField)).intValue();
    ...
    try {
      final Method arrayBaseOffsetM = unsafeClass.getMethod("arrayBaseOffset",
          Class.class);
      // we calculate that only for byte[] arrays, it's actually the same for
      // all types:
      arrayHeader = ((Number) arrayBaseOffsetM.invoke(theUnsafe, byte[].class))
          .intValue();
    ...
    JVM_INFO_STRING = "[JVM: " + Constants.JVM_NAME + ", "
        + Constants.JVM_VERSION + ", " + Constants.JVM_VENDOR + ", "
  ...
  public static long sizeOf(int[] arr) {
    return alignObjectSize((long) NUM_BYTES_ARRAY_HEADER + (long) NUM_BYTES_INT
        * arr.length);
  }
  ...
  private static long measureObjectSize(Object root) {
    ...
    while (!stack.isEmpty()) {
      ...
      if (obClazz.isArray()) {
         ...
         * references to the processing stack and accumulate this array's
         * shallow size.
         */
        long size = NUM_BYTES_ARRAY_HEADER;
        ...
        /*
         * Consider an object. Push any references it has to the processing
         * stack and accumulate this object's shallow size.
         */
        try {
  ...
  private static long adjustForField(long sizeSoFar, final Field f) {
    ...
    if (objectFieldOffsetMethod != null) {
      try {
        final long offsetPlusSize = ((Number) objectFieldOffsetMethod.invoke(
            theUnsafe, f)).longValue() + fsize;
